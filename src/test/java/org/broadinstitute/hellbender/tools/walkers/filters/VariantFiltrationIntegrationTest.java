...
public final class VariantFiltrationIntegrationTest extends CommandLineProgramTest {
    ...
    public void testInvertFilter() throws IOException {
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestString("vcfexample2.vcf", " -filterName ABF -filter 'AlleleBalance < 0.7' -filterName FSF -filter 'FisherStrand == 1.4' --" + VariantFiltration.INVERT_LONG_NAME + " "),
    ...
    public void testDeletions() throws IOException {
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestString("twoDeletions.vcf", " -filter 'QUAL < 100' -filterName foo "),
    ...
    public void testFilteringDPfromINFO() throws IOException {
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestString("filteringDepthInFormat.vcf", " -filter 'DP < 8' -filterName lowDP "),
    ...
    public void testSetFilteredGtoNocallUpdateInfo()  throws IOException {
        final IntegrationTestSpec spec = new IntegrationTestSpec(
                baseTestString("variantFiltrationInfoField.vcf", " -G_filter 'GQ < 20' -G_filterName lowDP -G_filter 'DP < 10' -G_filterName lowGQ --" + VariantFiltration.NO_CALL_GTS_LONG_NAME + " "),
